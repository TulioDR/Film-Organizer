...
export default function SeasonCard({ seasonInfo, openSeasonDetails }) {
   const { name, air_date, poster_path } = seasonInfo;
   const { episode_count, overview, season_number } = seasonInfo;
   return (
      <article className="flex rounded-lg shadow-material relative overflow-y-hidden bg-gray-200 dark:bg-gray-800 h-40 lg:h-52">
         <img
            src={getPoster(poster_path, "md", true)}
            alt={name}
            className="h-full"
         />
         <div className="p-4 pb-14 h-full overflow-y-auto text-sm scroller-scrollbar">
         ...
         <button
            ...
            className="text-sm absolute bottom-4 right-4 py-1 px-2 bg-blue-600 text-white rounded-md cursor-pointer"
         >
            Learn more
         </button>
      </article>
